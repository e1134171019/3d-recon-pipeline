...
def main():
    ...
    if args.min_opacity > 0.0:
        ...
        keep_mask &= opacity_linear >= args.min_opacity
        print(
            f"[FILTER] min_opacity={args.min_opacity:.4f} -> 保留 {int(keep_mask.sum()):,}/{len(keep_mask):,}"
        )
    if args.max_scale > 0.0:
        linear_scales = np.exp(scales)
        keep_mask &= linear_scales.max(axis=1) <= args.max_scale
        print(
            f"[FILTER] max_scale<={args.max_scale:.4f} -> 保留 {int(keep_mask.sum()):,}/{len(keep_mask):,}"
        )
