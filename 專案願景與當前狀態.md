...
| **下一步** | 4K PNG 正式重跑已完成：`train_4kpng_bestmcmc_aa_df2_30k_20260518` 的離線 LPIPS 為 **0.236957**，未打贏既有 JPG 基準 `0.189117`；但 Unity GUI 人工觀察顯示主體可辨識度較好、曝光仍偏高。下一個最高優先動作不是再掃 MCMC 小參數，而是建立 **offline metric / Unity deployment review 雙軌規則**，並把 4K rerun 補進 teacher / learner 回饋鏈 |
| **最後更新** | 2026-05-19 |
...
- **正式判讀規則**：
  ...
  - 後續 `P0` 規則必須把「離線 benchmark」與「Unity deployment review」分開；需要補正式 review label 後再進 teacher / learner。
