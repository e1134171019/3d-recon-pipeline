...
| **下一步** | `1M + antialiased` 升格為新的離線品質 benchmark；`750k + antialiased` 保留為現有最佳 Unity 候選；本輪 `MCMC` 小參數 probes（`ssim_lambda=0.3`、`depth_loss`、`with_ut`、`use_bilateral_grid`）已正式歸檔為負面結果，不升 full train；`Scaffold-GS` sandbox 已完成 300 sanity、7000 iter 放大訓練、30000 iter full train 與 Unity preview 資產驗證，正式結論仍是 offline 很強，但 Unity 畫面近景霧化 / 碎片化 / 結構不穩，問題主體轉為 `Scaffold -> Unity bridge`，而不是 trainer 參數；後續結果仍同步進 `Ollama/Qwen teacher -> offline learner -> 對話框 AI meta review` 鏈，但不升正式主線、不改 formal runtime |
| **最後更新** | 2026-05-13 |
