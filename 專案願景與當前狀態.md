...
| **下一步** | 4K PNG 正式重跑已完成：`train_4kpng_bestmcmc_aa_df2_30k_20260518` 的離線 LPIPS 為 **0.236957**，未打贏既有 JPG 基準 `0.189117`；Unity GUI 人工觀察已補成 `deployment_review.json`，最新結論是 **strong Unity candidate / not deployable pass**（場景完整度相當高，只剩少量霧化）。下一個最高優先動作不是再掃 MCMC 小參數，而是把後續實驗維持在 **offline metric / Unity deployment review 雙軌規則** 下，針對殘留霧化做確認或降低 |
| **最後更新** | 2026-05-20 |
...
- **正式判讀規則**：
  ...
  - 後續 `P0` 規則必須把「離線 benchmark」與「Unity deployment review」分開；4K review label 已補成正式 artifact，但仍不得用單次 GUI 觀察覆寫正式主線。
