...
| **下一步** | 4K PNG 正式重跑已完成：`train_4kpng_bestmcmc_aa_df2_30k_20260518` 的離線 LPIPS 為 **0.236957**，未打贏既有 JPG 基準 `0.189117`；Unity GUI 人工觀察已補成 `deployment_review.json`，且 2026-05-22 新增 `unity_deployment_review_20260522/deployment_review_score.json` 自動多視角 gate，結果為 **deployment_review_pass=true / deployable_pass=false / human_review_required=true**。下一個最高優先動作不是再掃 MCMC 小參數，而是把後續實驗維持在 **offline metric / Unity deployment review 雙軌規則** 下，補上 reference-image 對照或人工 acceptance，才可討論 deployable promotion |
| **最後更新** | 2026-05-22 |

---

## 🔎 最新正式重跑狀態（2026-05-18 / 2026-05-19 補記）

- **資料來源**：`data/frames_4k_png`
- **SfM event**：`outputs_sfm_complete_20260517_211925`
  - `images_count = 870`
  - `registered_images_count = 870`
  - `points3d_count = 160413`
  - `can_proceed_to_3dgs = true`
- **Train event**：`outputs_train_complete_20260518_014004`
  - run：`outputs/experiments/formal_reruns/train_4kpng_bestmcmc_aa_df2_30k_20260518`
  - params：`MCMCStrategy + cap_max=1M + antialiased + data_factor=2 + 30000 iter`
  - metrics：`PSNR 27.2504 / SSIM 0.8925 / LPIPS 0.236957 / num_GS 1,000,000`
- **Export event**：`outputs_export_complete_20260518_015025`
  - Unity PLY：`outputs/experiments/formal_reruns/train_4kpng_bestmcmc_aa_df2_30k_20260518/unity_view/point_cloud_unity.ply`
  - `ply_size_mb = 236.513`
- **Deployment review**：`outputs/experiments/formal_reruns/train_4kpng_bestmcmc_aa_df2_30k_20260518/deployment_review.json`
  - verdict：`unity_result = strong_candidate`
  - `deployment_review_pass = false`
  - `deployable_pass = false`
  - 判讀：場景完整度相當高，只剩少量霧化；目前是 strong Unity candidate，但在殘留霧化被降低或多視角 deployment review 接受前，不可升格為正式可交付底座
- **Automatic Unity deployment review**：`outputs/experiments/formal_reruns/train_4kpng_bestmcmc_aa_df2_30k_20260518/unity_deployment_review_20260522/deployment_review_score.json`
  - `views_evaluated = 8`
  - `valid_views = 8`
  ...
  - `deployable_pass = false`
  - 判讀：自動多視角技術 gate 已補上並通過，但目前仍要求 human / reference-image review，不得只靠自動閾值升格為可交付底座
- **正式判讀規則**：
  - 這筆 4K rerun **不改寫離線最佳 benchmark**，因為 LPIPS 沒有打贏 `1M + antialiased + MCMC @ 30000` 的 `0.189117`。
  - 這筆 4K rerun 也**不能被視為失敗**，因為輸入解析度與來源壓縮條件已改變，且 Unity GUI 人工觀察有正向訊號。
  - 後續 `P0` 規則必須把「離線 benchmark」與「Unity deployment review」分開；4K review label 與 automatic review score 都已補成 artifact，但仍不得用單次 GUI 觀察或未校準的自動閾值覆寫正式主線。
...
- **Agent 的決策職責**：Agent 需要讀取不同階段的指標（SfM 註冊率、5000 iter 早期指標等），在每次 Rerun 前提出 advisory 參數建議；決策層審計輸出使用 `sfm_params_advisory.json` / `train_params_advisory.json`，不得直接定義生產層真相。
- **正式接線方式**：生產層會在 stage 完成後寫出 `outputs/agent_events/latest_*.json` contract；決策層只讀這些 contract，不再依賴舊式固定路徑掃描。`train_complete` / `export_complete` 現在會在寫出 contract 後，透過 `AGENT_TEST_RUNNER` 或 `AGENT_TEST_ROOT` 指向的 `run_phase0.py --contract ...` 嘗試刷新 `outputs/agent_decisions/latest_*_decision.json`；未設定時只回報 `agent_runner_unconfigured` 並跳過 hook，不再硬編碼本機路徑。若 decision hook 失敗，生產層會保留 event 與本地 contract，改以警告呈現，不讓決策層異常反向炸掉主流程。
