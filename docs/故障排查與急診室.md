...
### Step 3：Automatic Unity deployment review（固定視角截圖 + score）

> 目的：把 Unity GUI 人工觀察改成可重放 artifact。這是 deployment review gate，不是訓練指標；不得用它覆寫離線 LPIPS benchmark。

```powershell
$PLY = "C:\3d-recon-pipeline\outputs\experiments\formal_reruns\train_4kpng_bestmcmc_aa_df2_30k_20260518\unity_view\point_cloud_unity.ply"
$OUT = "C:\3d-recon-pipeline\outputs\experiments\formal_reruns\train_4kpng_bestmcmc_aa_df2_30k_20260518\unity_deployment_review_20260522"

.\scripts\run_unity_deployment_review.ps1 `
  -SourcePly $PLY `
  -OutputDir $OUT `
  -AssetBaseName "formal_4kpng_bestmcmc_aa_df2_30k_20260518_point_cloud_unity" `
  -Width 1280 `
  -Height 720
```
...
- `deployment_review_score.json`
- `unity_import.log`
- `unity_deployment_review.log`
- `screenshots/*.png`

判讀規則：

- `deployment_review_pass=true` 只代表 automatic Unity gate 通過。
- `deployable_pass=false` 仍可同時成立，代表還需要 reference-image 對照或人工 acceptance。
- `human_review_required=true` 時，不得把結果直接升格正式可交付底座。
