...
### Phase 0 v2 歷史架構與現行定位

> 2026-04-27 清理結論：`src/phase0_runner_v2.py` 與 `src/run_ffmpeg_fullchain.py` 已從生產層移除。  
> `ffmpeg full-chain` 曾證明流程可跑通，但最終沒有打贏目前 `U_base + MCMC` 主線，因此不保留獨立 launcher。  
> 若未來要重啟 Phase 0 v2，必須把有用能力重新整合進正式六主程式中的 `preprocess_phase0.py` / `downscale_frames.py`，不得恢復舊的獨立入口。
...
| `preprocess_phase0.py` / `downscale_frames.py` | 若 Phase 0 v2 重啟，所有可用能力必須合併回正式 Phase 0 主程式，而不是新增獨立 runner | `data/frames_cleaned/` / `data/frames_1600/` |
