...
def _get_matcher_time_estimate(use_gpu: bool) -> str:
    """
    返回 VocabTree Matcher 的保守時間估計（非精確預測）
    ...
    Returns:
        人類可讀的時間估計字符串
    """
    # 注意：實際時間受硬件、場景複雜度等多因素影響，此估計不精確
    if use_gpu:
        return "5-10 分鐘（GPU 加速）"
    else:
        # CPU 模式：實際根據硬件配置變化 30-120 分鐘
        return "30-60 分鐘（CPU 模式，實際可能更長）"
...
def check_matching(db: str) -> dict:
    ...
    # 關鍵判定: 內點率 > 0.1 (GitHub 官方推薦🔴)
    if inlier_ratio < 0.1:  # ← 新增關鍵⭐⭐⭐
        errors.append(
            f"內點率過低: {inlier_ratio:.3f} < 0.1 (特徵品質差，匹配結果多為外點)"
        )
    ...
    if inlier_ratio < 0.05:
        warnings.append(
            f"WARNING: 內點率很低 ({inlier_ratio:.3f}), 可能導致 Mapper 失敗"
        )
...
def check_reconstruction(sparse_model_dir: str) -> dict:
    ...
    # 2. 嘗試使用 pycolmap 讀取統計，降級到檔案大小估計
    stats = {
        "cameras_count": 0,
        "images_count": 0,
        "points3d_count": 0,
    ...
    try:
        import pycolmap
        # pycolmap 1.5.3 uses SceneManager with direct attribute access
        scene_manager = pycolmap.SceneManager(str(sparse_path))
        stats["cameras_count"] = len(scene_manager.cameras)
        stats["images_count"] = len(scene_manager.images)
        stats["points3d_count"] = len(scene_manager.points3D)
    except:
        # Fallback: estimate based on file sizes
        # Heuristics: 
        #   cameras.bin: ~136 bytes per camera
        #   images.bin: ~320 bytes per image
        #   points3D.bin: ~45 bytes per point
        try:
            ...
            stats["cameras_count"] = max(1, cameras_size // 136) if cameras_size > 0 else 1  # 至少假设 1 个相机（单相机模式）
            stats["images_count"] = max(1, images_size // 320) if images_size > 0 else 0
            stats["points3d_count"] = max(1, points3d_size // 45) if points3d_size > 0 else 0
            
            warnings.append("Using file size estimation (pycolmap not available)")
        except:
            errors.append("Cannot read statistics (pycolmap unavailable and file estimation failed)")
    
    # 3. 驗證重建品質
    min_cameras = 3             # COLMAP 官方最小值
    min_points3d = 50000        # 我們設定的目標
    
    if stats["cameras_count"] < min_cameras:
        errors.append(
            f"相機數太少: {stats['cameras_count']} < {min_cameras} (匹配品質不足)"
        )
...
@app.command()
def main(
    imgdir: str = "data/frames_cleaned",
    work: str = "outputs/SfM_models",
    use_gpu: bool = True,         # 改為 True（已安裝 GPU 版 COLMAP）
    max_image_size: int = 3000,   # 保留原始解析度以獲得更多特徵
    max_features: int = 16000,    # SIFT 最大特徵數
    ...
    # 統一 GPU 標志（供 feature_extractor 和 vocab_tree_matcher 共用）
    ...
    try:
        ...
        print("接下來：Step 2 匹配驗證（VocabTree 索引 + SIFT 匹配）\n")
        print(f"估計時間：{matcher_estimate}\n")

        # 2) VocabTree 匹配（全局相似性檢索，FAISS 格式）
        if not (resume and Path(db).exists() and Path(db).stat().st_size > 20 * 1024 * 1024):
            ...
            run([
                ...
                "--VocabTreeMatching.vocab_tree_path", str(Path(__file__).parent.parent / "colmap" / "vocab_tree_flickr100K_words32K.bin"),
            ])
        ...
        sparse0 = work_p / "sparse" / "0"
        if not sparse0.exists():
            raise SystemExit(
                f"❌ COLMAP Mapper 失敗診斷\n\n"
                f"預期輸出目錄不存在：{sparse0.resolve()}\n\n"
                f"檢查清單：\n"
        ...
        # ✓ Step 3 檢查：重建驗證
        result3 = check_reconstruction(str(sparse0))
        if not result3["pass"]:
            raise SystemExit(
                "Step 3 重建驗證失敗。可能原因：\n" +
        ...
        # ── 導出驗證報告供決策層使用 ──────────────────────
        print("\n[*] 正在導出點雲驗證報告...")
        export_signals(result3, str(sparse0.resolve()), reports_dir)
        ...
        print(f"  ✓ Step 3 重建驗證: {result3['cameras_count']} 相機, {result3['points3d_count']:,} 點雲")
        print("\n🚀 下一步：")
        if result3['can_proceed_to_3dgs']:
            print("  ➜ 可以進行 3DGS 訓練 [train_3dgs.py]")
            print(f"     稀疏模型: {sparse0.resolve()}")
